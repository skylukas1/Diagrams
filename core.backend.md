# core.backend - Production Deployment

> **Namespace:** `production` | **Cluster:** `eks-production` | **Service Account:** `yardstik-production` | **Created:** `02/17/2026`

---

## 1. Network

All virtual services use **Gloo Gateway** with TLS v1.2 minimum and the `star.yardstik.com` certificate. HTTP requests are redirected to HTTPS on all domains.

### Virtual Service Summary

| Domain | Path | Upstream | Timeout | Retries | Per-Try Timeout | Retry On | Notable Options |
|--------|------|----------|---------|---------|-----------------|----------|-----------------|
| `api.yardstik.com` | `/twilio/voice/reply` | _(direct response 200)_ | - | - | - | - | Returns XML voice response |
| `api.yardstik.com` | `/vite` | `production-core-backend-3000` | - | - | - | - | `Cache-Control: max-age=31536000, immutable` |
| `api.yardstik.com` | `/bullhorn/{hash}/tab` | `production-core-backend-3000` | - | - | - | - | CSP with `frame-ancestors bullhornstaffing.com` |
| `api.yardstik.com` | `/` | `production-core-backend-3000` | 60s | 3 | 10s | connect-failure, refused-stream, reset | HSTS, removes `server` header |
| `hub2.yardstik.com` | `/` | `production-core-backend-3000` | 60s | 3 | 10s | cancelled, reset, unavailable, gateway-error, connect-failure, refused-stream, retriable-status-codes | CORS enabled, WAF (Log4j CVE, CloudFront header checks), CSP |
| `admin.yardstik.com` | `/` | `production-core-backend-3000` | 120s | 2 | 60s | connect-failure, refused-stream, reset | CSP, X-Frame-Options SAMEORIGIN |
...
graph LR
    subgraph Internet
        C1([api.yardstik.com])
        C2([hub2.yardstik.com])
        C3([admin.yardstik.com])
    end

    subgraph "Gloo Gateway (gloo-system)"
        VS1["VirtualService<br/>core-backend-production"]
        VS2["VirtualService<br/>hub2-production"]
        VS3["VirtualService<br/>active-admin-production"]
    end

    UP["Upstream<br/>production-core-backend-3000"]
...
| `core-backend-hutch` | 5 | `bundle exec hutch` | 50m / 650m | 300Mi / 1000Mi | - | - | `rake db:abort_if_pending_migrations` |
| `core-backend-sidekiq-15s` | 2 | `bundle exec sidekiq -q latency_15s` | 50m / 500m | 300Mi / 1000Mi | - | HTTP `:7433/` (30s interval, 30s delay) | `rake db:abort_if_pending_migrations` |
...
|-----|---------|---------|---------------|--------------------|
| `db-migrate` | `rails db:migrate --trace` | ArgoCD Sync Hook | 3 | 3600s |
    ...
    subgraph EKS["EKS Cluster: eks-production"]
        subgraph NS["Namespace: production"]
            ...
            subgraph D2["Deployment: core-backend-hutch"]
                P2["Pod x5<br/>bundle exec hutch<br/>CPU: 50m-650m | Mem: 300Mi-1000Mi"]
            end

            subgraph D3["Deployment: sidekiq-15s"]
                P3["Pod x2<br/>sidekiq -q latency_15s<br/>CPU: 50m-500m | Mem: 300Mi-1000Mi"]
            end
            ...
            subgraph JOB["Job: db-migrate (ArgoCD Sync)"]
                J1["rails db:migrate --trace<br/>backoffLimit: 3"]
            end
        end
    end
```
...
|--------------|----------------|----------------|---------|
| `core-backend-adverse-emails` | `0 10 * * *` | 4:00 AM CST daily | `rake adverse_actions:adverse_emails` |
| `core-backend-contact-verifications-expire` | `0 6 * * *` | 12:00 AM (midnight) CST daily | `rake contact_verifications:expire` |
| `core-backend-contact-verifications-reminder` | `0 17 * * *` | 11:00 AM CST daily | `rake contact_verifications:send_reminder_email` |
...
| `core-backend-send-reminder-email` | `0 15 * * *` | 9:00 AM CST daily | `rake invitations:send_reminder_email` |
...
graph TB
    subgraph EKS["EKS Cluster: eks-production"]
        ...
        subgraph DAILY["Daily Jobs"]
            ...
            D3["invitations-expire<br/>11:00 PM CST<br/>rake invitations:expire"]
            D4["import-cleanup<br/>11:10 PM CST<br/>rake imports:remove_old_documents"]
            D5["contact-verifications-expire<br/>12:00 AM CST<br/>rake contact_verifications:expire"]
            D6["adverse-emails<br/>4:00 AM CST<br/>rake adverse_actions:adverse_emails"]
            D7["send-reminder-email<br/>9:00 AM CST<br/>rake invitations:send_reminder_email"]
            D8["info-request-reminder<br/>9:00 AM CST<br/>rake info_request:reminder"]
            D9["contact-verifications-reminder<br/>11:00 AM CST<br/>rake contact_verifications:send_reminder_email"]
        end
    end
```
